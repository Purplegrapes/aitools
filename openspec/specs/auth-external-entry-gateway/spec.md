...
- **AND** 中转入口 SHALL 使用 `referrer` 或等价参数保留目标页信息
...
系统 SHALL 在网关迁移到 `auth` 后继续保持现有小程序来源识别、`transferH5Ticket` 换 token、失败分流和 referrer 直跳行为语义不变。

#### Scenario: Miniapp external request contains required gateway params
- **WHEN** 用户通过小程序外部来源进入 `auth` 网关且 query 中包含 `transferH5Ticket` 与 `referrer`
- **THEN** 系统 SHALL 自动发起一次 `transferH5Ticket` 换 token 请求
- **AND** 请求成功后 SHALL 写入统一登录态
- **AND** 系统 SHALL 在换票完成后直接跳转到 `referrer` 指向的目标地址

#### Scenario: Miniapp external request misses required gateway params
- **WHEN** 小程序外部来源进入 `auth` 网关但缺少 `transferH5Ticket` 或 `referrer`
- **THEN** 系统 SHALL 进入明确失败分流
- **AND** 系统 SHALL 不继续执行业务目标页跳转

#### Scenario: Referer is a full website URL
- **WHEN** 网关接收到的 `referrer` 是完整的 `https://` 或 `http://` 网站地址
